      module mod_poten_pair_lj

C     Purpose: Has routines for computing forces for pair potential.
C     Currently relies on linear interpolation of tabular data (force
C     vs. distance), but could easily be extended to analytical
C     functions (e.g. LJ/Morse)
C     
C     Possible extensions: Compute energies for pair potential? (For
C     molecular statics). Also, implement LJ/Morse?
      ...
      private
      public :: getPotForceTable, getPotForcesAll
      
      contains
************************************************************************
      subroutine getPotForcesAll()
      
C     Subroutine: getPotForcesAll
...
C     local variables
     ...
     &                        nodes%potforces(:,neigh) - forcevec(1:2)
      ...
      end subroutine getPotForcesAll
************************************************************************  
      function getPotForceLJ(pnum,atompos,neighpos) result(forcevec)
      
C     Function: getPotForceLJ
