...
C     equations for "hat" fields. Uses dislocation fields to get correct boundary conditions.
...
C     Possible extensions: Include routine for applied
C     nodal forces (using groups?)

      use mod_types, only: dp
      use mod_utils, only: prettyPrintRealMat
      use mod_math, only: getUnitNormalRHR
      use mod_fe_el_2d, only: getK_2d, getN_2d, getB_2d
     ...
     &  getFEStrainAtPoint, getFEDispAtPoint, updateNodalPosn, assembly
...
C     and solving K*u = f for each, and subsequently updating FE node
C     positions, displacements.
...
C     local variables
      ...
      do i = 1, nfematerials
          ...
          call getDislForceRHS(i,nedgenodes,nedgeip)
          call getDispRHS(i)
          call solveOneMat(i)
          call updateNodalPosn(i)
      end do
...
C     Function: assembleAndFactorAll
...
C     local variables
      ...
      do i = 1, nfematerials       
          ...
          neldof = feelements(i)%neldof
          nelip = feelements(i)%nelip
          
          call assembleKNormal(i,C,nelnodes,neldof,nelip)
...
************************************************************************      
      subroutine assembleKNormal(mnumfe,C,nelnodes,neldof,nelip)

C     Function: assembleKNormal

C     Inputs: mnumfe --- number of continuum (FE) material
C             C --- stiffness matrix of matrix
C             nelnodes --- number of nodes per element
C             neldof --- number of degrees of freedom per element
...
C     local variables
      ...
      centries = 0
      rgauss = feelements(mnumfe)%rgauss
      sgauss = feelements(mnumfe)%sgauss
      wgauss = feelements(mnumfe)%wgauss
...
C             use undeformed positions
          ...
          K = getK_2d(posn,C,neldof,nelnodes,nelip,
     &                feelements(mnumfe)%elname,rgauss,sgauss,wgauss)
          do k1 = 1, neldof
          do k2 = 1, neldof
C             just upper triangle
              if (k1 <= k2) then
...
************************************************************************    
      subroutine getDislForceRHS(mnumfe,nedgenodes,nedgeip)
      
C     Function: getDislForceSub

C     Inputs: mnumfe --- number of continuum (FE) material
C             nedgenodes --- number of nodes per edge of element
C             nedgeip --- number of integration points on edge (for line integral)
...
C     local variables
      integer :: i, j
      integer :: node
      integer :: nodeidx(nedgenodes)
      integer :: nodetype, bcflag, idx
      logical :: allfixed
      logical :: xfixed(nedgenodes), yfixed(nedgenodes)
      real(dp) :: posn(2,nedgenodes)
      real(dp) :: fx(nedgenodes), fy(nedgenodes)
      real(dp) :: sgauss(nedgeip)
      real(dp) :: wgauss(nedgenodes,nedgeip)
      
      sgauss = feelements(mnumfe)%sedgegauss
      wgauss = feelements(mnumfe)%wedgegauss
...
C             use undeformed positions
          ...
          if (.not.allfixed) then
              call getDislForceSub(mnumfe,nedgenodes,nedgeip,posn,
     &                             sgauss,wgauss,fx,fy)
...
************************************************************************       
      subroutine getDislForceSub(mnumfe,nedgenodes,nedgeip,posn,
     &                         sgauss,wgauss,fx,fy)
...
C             sgauss --- locations of integration points along edge (0 <= s <= 1)
C             wgauss --- weights relating nodal forces to tractions at integration points,
...
C     local variables
      ...
      real(dp) :: stress(3)
      real(dp) :: tracx(nedgeip), tracy(nedgeip)
      real(dp) :: edgelen
      ...
      do i = 1, nedgeip
          posnip = posn(:,1) + posndiff*sgauss(i)
      ...
      fx = edgelen*matmul(wgauss,tracx)
      fy = edgelen*matmul(wgauss,tracy)
...
************************************************************************
      function getFEDispAtPoint(mnumfe,element,nelnodes,r,s)
     &                                                     result(disp)
...
C     local variables
      integer :: i
      real(dp) :: N(nelnodes)
      real(dp) :: udisp(nelnodes)
      real(dp) :: vdisp(nelnodes)
      integer :: node, nodeidx

      N = getN_2d(r,s,feelements(mnumfe)%elname,nelnodes)
      do i = 1, nelnodes
...
************************************************************************
      function getFEStrainAtPoint(mnumfe,element,nelnodes,neldof,r,s)
     &                                                   result(strain)
...
C     local variables
      ...
      call getB_2d(posn,r,s,neldof,nelnodes,feelements(mnumfe)%elname,
     &             B,J)
      strain = matmul(B,disp)
      
      end function getFEStrainAtPoint
************************************************************************
      subroutine updateNodalPosn(mnumfe)

C     Subroutine: updateNodalPosn

C     Inputs: mnumfe --- number of continuum (FE) material

C     Outputs: None

C     Purpose: Update FE nodal positions, displacements using FE solution
C     (from assembly(mnumfe)%(rhs))
...
C     local variables
      ...
      do i = 1, size(feelements(mnumfe)%nodelist)
          ...
          if (.not.xfixed) then
              ...
              nodes%posn(1,node) = nodes%posn(1,node)
     &                           + (udisp - nodes%posn(4,node))
              nodes%posn(4,node) = udisp
          end if
          if (.not.yfixed) then
              vdisp = assembly(mnumfe)%rhs(2*i)
              nodes%posn(2,node) = nodes%posn(2,node)
     &                           + (vdisp - nodes%posn(5,node))
              nodes%posn(5,node) = vdisp
          end if
      end do    
      
      end subroutine updateNodalPosn
************************************************************************
      end module   
