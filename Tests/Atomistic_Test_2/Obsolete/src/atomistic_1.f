      ...
      use mod_io, only: writeOutput, initAtomistic, writeDump
...
C         call updateNeighbors(.false.)
      ...
      contains
************************************************************************
      subroutine runNVE(gnum,increments)
...
C     initialize all forces
      ...
      call getDampingForcesAll(damping%gnum,damping%gamma,damping%flag)
      
C     run verlet loop
      do i = 1, increments
          if (isMultiple(misc%incrementcurr,100,.false.)) then
              write(*,*) 'i', i
          end if    
          call updateNeighbors()
          call velocityVerletSubAll(misc%timestep,gnum,
     &             damping%gnum,damping%flag,damping%gamma)
